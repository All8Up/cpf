...
namespace CPF
{
	namespace SIMD
	{
		...
		{
			template <int COUNT>
			inline Bool4_<COUNT> CPF_VECTORCALL operator == (const F32x4_<COUNT> lhs, const F32x4_<COUNT> rhs)
			{
				return Bool4_<COUNT>(_mm_castps_si128(
					_mm_cmpeq_ps(static_cast<__m128>(lhs), static_cast<__m128>(rhs))));
			}
			inline Bool4_<1> CPF_VECTORCALL operator == (const F32x4_<1> lhs, const typename F32x4_<1>::LaneType rhs)
			{
