...
PriorityData sWorkPriorityData[]
{
	{ 512, 1 },
	{ 128, 1 },
	{ 128, 0 },
...
void CPF_STDCALL Scheduler::SetPriority(SchedulingPriority level)
{
	...
	for (int i=0; i<kMaxThreads; ++i)
	{
		...
		mWorkBackoff[i].SetMaxSpin(sWorkPriorityData[int(level)].mSpinCount);
		mWorkBackoff[i].SetSleepMS(sWorkPriorityData[int(level)].mSleepMS);
	}
}
...
bool Scheduler::_FetchWork()
{
	...
	if (externalCount > 0)
	{
		auto pullCount = mInstructionRing.Reserve(mActiveCount.load(), mQueueSize-1 < externalCount ? mQueueSize - 1 : externalCount);
		if (pullCount == 0)
			return false;
