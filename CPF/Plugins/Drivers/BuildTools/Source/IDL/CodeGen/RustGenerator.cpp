...
//////////////////////////////////////////////////////////////////////////
CPF::STD::String ConvertName(const CPF::STD::String& name)
{
	std::locale loc;
...
void RustGenerator::Begin(Visitor& visitor, CodeWriter& writer)
{
	...
	visitor.On<Visitor::Start>(CPF::STD::Bind(&RustGenerator::OnStart, this));
	visitor.On<Visitor::ModuleStmt>(CPF::STD::Bind(&RustGenerator::OnModule, this, _1));
	visitor.On<Visitor::SuccessType>(CPF::STD::Bind(&RustGenerator::OnSuccessType, this, _1, _2, _3));
	visitor.On<Visitor::FailureType>(CPF::STD::Bind(&RustGenerator::OnFailureType, this, _1, _2, _3));
	visitor.On<Visitor::ImportFromStmt>(CPF::STD::Bind(&RustGenerator::OnImportStmt, this, _1, _2));
	visitor.On<Visitor::InterfaceDeclStmt>(CPF::STD::Bind(&RustGenerator::OnInterfaceDeclStmt, this, _1));
	visitor.On<Visitor::EnumDeclStmt>(CPF::STD::Bind(&RustGenerator::OnEnumDeclStmt, this, _1));
}
...
CPF::STD::String RustGenerator::TypeToString(const Visitor::Type& decl)
{
	String result;
	switch (decl)
	{
...
CPF::STD::String RustGenerator::TypeToString(const Visitor::TypeDecl& decl)
{
	String result;
	...
	case Visitor::Type::Ident:
	{
		for (auto it = decl.mIdent.begin(); it != decl.mIdent.end(); ++it)
		{
			...
			String p = *it;
			if (!last)
				CPF::STD::Transform(p.begin(), p.end(), p.begin(), [](char c) { return (char)::tolower(c); });
			result += p + (last ? "" : "::");
		}
	}
	break;
