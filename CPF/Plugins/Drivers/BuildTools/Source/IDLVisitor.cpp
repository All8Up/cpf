...
//////////////////////////////////////////////////////////////////////////
CPF::STD::String TrimStringLit(const CPF::STD::String& lit)
{
	if (lit.empty())
		return CPF::STD::String();
	return CPF::STD::String(lit.begin() + 1, lit.end() - 1);
}

CPF::STD::String TrimCharLit(const CPF::STD::String& lit, bool& isEscape)
{
	isEscape = false;
	CPF::STD::String result = TrimStringLit(lit);
	if (result[0] == '\\')
	{
		isEscape = true;
		return CPF::STD::String(result.begin() + 1, result.end());
	}
	return result;
}

int64_t ParseOctal(const CPF::STD::String& lit)
{
	int64_t result = 0;
...
SymbolPath ParseQualifiedIdent(IDLParser::Qualified_identContext* ctx)
{
	...
	{
		path.Push(CPF::STD::String(p->IDENT()->toString().c_str()));
	}
	return path;
}
...
CPF::Outcome<Visitor::FunctionDecl, CPF::STD::String> Visitor::HandleFunctionDecl(IDLParser::Function_declContext* context)
{
	using ResultType = CPF::Outcome<FunctionDecl, CPF::STD::String>;

	if (context == nullptr)
		return ResultType::Error(CPF::STD::Move("Not a valid context."));

	FunctionDecl functionDecl;
	functionDecl.mName = context->IDENT()->toString();
	functionDecl.mReturnType = ParseTypeDecl(context->type_decl());
	if (context->function_param_list())
	{
		for (auto param : context->function_param_list()->function_param())
		{
			ParamDecl paramDecl;
			paramDecl.mName = param->IDENT() ? param->IDENT()->toString() : CPF::STD::String();
			paramDecl.mType = ParseTypeDecl(param->type_decl());
			functionDecl.mParams.push_back(paramDecl);
		}
	}
	if (context->Const())
		functionDecl.mConst = true;
	return ResultType::OK(CPF::STD::Move(functionDecl));
}
...
antlrcpp::Any Visitor::visitInterface_decl(IDLParser::Interface_declContext *ctx)
{
	...
	for (const auto& item : ctx->interface_block()->interface_item())
	{
		...
		else if (item->function_decl())
		{
			...
			if (HandleFunctionDecl(item->function_decl()).CheckOK(funcDecl))
			{
...
Visitor::MemberInitValue Visitor::ParseInitValue(IDLParser::Member_init_valueContext* ctx)
{
	...
	else if (ctx->float_lit())
	{
		...
		std::string floatString(ctx->float_lit()->FLOAT_LIT()->toString());
		sscanf(floatString.c_str(), "%f", &value.mFloat);
	}
...
antlrcpp::Any Visitor::visitConst_integral_def(IDLParser::Const_integral_defContext *ctx)
{
	...
	if (ctx->const_integral_as())
		value.mAsType = ParseIntegralType(ctx->const_integral_as()->integral_type());
	else
		value.mAsType = Type::None;
...
Visitor::TypeDecl Visitor::ParseTypeDecl(IDLParser::Type_declContext* typeDecl)
{
	...
	else if (anyType->option_type())
	{
		result.mType = Type::Option;
		TypeDecl resultType = ParseTypeDecl(anyType->option_type()->type_decl());
		result.mChildTypes.push_back(std::make_shared<TypeDecl>(resultType));
	}
	else if (anyType->outcome_type())
	{
		result.mType = Type::Outcome;
		TypeDecl resultType = ParseTypeDecl(anyType->outcome_type()->type_decl());
		result.mChildTypes.push_back(std::make_shared<TypeDecl>(resultType));
		if (anyType->outcome_type()->outcome_error())
		{
			result.mChildTypes.push_back(std::make_shared<TypeDecl>(
				ParseTypeDecl(anyType->outcome_type()->outcome_error()->type_decl())
				));
		}
	}
	else
...
CPF::STD::String Visitor::ParseExprValue(IDLParser::Expr_valueContext* expr)
{
...
CPF::STD::String Visitor::ParseExprLogical(IDLParser::Expr_logicalContext* expr)
{
	if (expr->PIPE())
	{
...
CPF::STD::String Visitor::ParseExprShift(IDLParser::Expr_shiftContext* expr)
{
	if (expr->LSHIFT())
	{
...
CPF::STD::String Visitor::ParseExprMulDiv(IDLParser::Expr_mul_divContext* expr)
{
	if (expr->STAR())
	{
...
CPF::STD::String Visitor::ParseExprAddSub(IDLParser::Expr_add_subContext* expr)
{
	if (expr->PLUS())
	{
...
CPF::STD::String Visitor::ParseEnumExpr(IDLParser::Enum_exprContext* expr)
{
