...
namespace CPF
{
	...
	namespace Driver
	{
		namespace D3D12
		{
			...
			class CommandBuffer final : public GOM::tUnknown<Graphics::iCommandBuffer>
			{
			public:
				...
				void CPF_STDCALL ClearDepthStencilView(Graphics::iImageView* view, Graphics::DepthStencilClearFlag flags, float depth, uint32_t stencil, int32_t count, const Math::Rectanglei* rects);
			...
			private:
				...
				using CommandListVector = STD::Vector<CommandListPtr>;
				using CommandListStack = STD::Vector<CommandListVector>;
				...
				STD::UnorderedSet<ID3D12DescriptorHeap*> mHeaps;
				Graphics::RenderPassBeginDesc mRenderPass;
				STD::Vector<Graphics::ResourceState> mAttachmentStates;
