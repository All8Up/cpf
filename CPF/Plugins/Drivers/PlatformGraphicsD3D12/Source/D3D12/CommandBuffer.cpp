...
#include "CPF/Platform/Graphics/DepthStencilClearFlag.hpp"
...
CommandBuffer::CommandBuffer(Plugin::iRegistry*, GOM::iUnknown*)
	...
	, mCurrent(-1)
	, mSubPass(0)
	, mType(Graphics::CommandBufferType::ePrimary)
	, mRenderPass{0}
	, mColorViewCount(0)
...
void CPF_STDCALL CommandBuffer::ClearDepthStencilView(Graphics::iImageView* view, Graphics::DepthStencilClearFlag flags, float depth, uint32_t stencil, int32_t count, const Math::Rectanglei* rects)
{
...
GOM::Result CommandBuffer::_StartSubPass()
{
	...
	if (rPass && fBuffer)
	{
		...
		if (renderPass->GetRenderPassDesc().mSubPasses.size() > mSubPass)
		{
			...
			{
				// TODO: This will fail to generate the correct states in subpasses when
				// they become more complex, specifically post processing will not work
				// with the logic here.  The transitions need to be split into two
				// pieces: this portion moves from subpass to subpass states while
				// the end render pass transitions to the final states.
				const Graphics::ImageAndView& target = frameBuffer->GetImages()[color.mIndex];
				if (mAttachmentStates[color.mIndex] != color.mState)
				{
					_MakeBarrier(
			...
			if (subPass.mDepthStencilAttachment.mIndex != Graphics::kInvalidAttachment)
			{
				...
				if (mAttachmentStates[subPass.mDepthStencilAttachment.mIndex] != subPass.mDepthStencilAttachment.mState)
				{
					_MakeBarrier(
						barrier + currentIndex,
						static_cast<Image*>(target.mpImage),
			...
			//////////////////////////////////////////////////////////////////////////
			// Issue clears, only during first subpass.
			for (const auto& color : subPass.mColorAttachments)
			{
				const Graphics::ImageAndView& target = frameBuffer->GetImages()[color.mIndex];
				if (mSubPass == 0 && attachments[color.mIndex].mLoadOp == Graphics::LoadOp::eClear)
				{
					CPF_ASSERT(mRenderPass.mpClearValues && mRenderPass.mpClearValues[color.mIndex].mDataType == Graphics::ClearValueType::eColor);
					Math::Vector4fv c(
						mRenderPass.mpClearValues[color.mIndex].mData.mColor.mColor[0],
						mRenderPass.mpClearValues[color.mIndex].mData.mColor.mColor[1],
						mRenderPass.mpClearValues[color.mIndex].mData.mColor.mColor[2],
						mRenderPass.mpClearValues[color.mIndex].mData.mColor.mColor[3]);
			...
			if (subPass.mDepthStencilAttachment.mIndex != Graphics::kInvalidAttachment)
			{
				...
				const Graphics::ImageAndView& target = frameBuffer->GetImages()[subPass.mDepthStencilAttachment.mIndex];
				if (mSubPass == 0 && attachments[subPass.mDepthStencilAttachment.mIndex].mLoadOp == Graphics::LoadOp::eClear)
				{
					ClearDepthStencilView(
						target.mpImageView,
						Graphics::DepthStencilClearFlag::eDepth,
						mRenderPass.mpClearValues[subPass.mDepthStencilAttachment.mIndex].mData.mDepthStencil.mDepth,
						mRenderPass.mpClearValues[subPass.mDepthStencilAttachment.mIndex].mData.mDepthStencil.mStencil,
						0,
						nullptr);
				}
			}
